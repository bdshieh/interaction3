...
array = field.xdc_linear_array(16, 92e-6, 1e-2, 10e-6, 1, 1, np.array([0, 0, 300]))
...
synthetic_rf = synthetic_rf.reshape((-1, 16, 16), order='F')

angular_rf, angular_t0 = list(), list()
for a in angles:

    x = 300 * np.sin(np.deg2rad(a))
    y = 0
    z = 300 * np.cos(np.deg2rad(a))
