...
#     nnodes = len(nodes)
#     h_gap = props['gap']
#     h_isol = props['isolation']
#     e_r = props['permittivity']
#
#     K = make_k(dx, dy, nnodes, props)
#
#     if sps.issparse(K):
#         Kinv = linalg.inv(K.todense())
#     else:
...
#     collapse_v = 0
#
#     for dc_bias in range(0, 100):
#
#         while True:
#
#             err = K.dot(u0) - pressure_es(u0, dc_bias, h_eff, e_flag)
#
#             if np.max(np.abs(err)) < tol:
#                 break
#
...
#                 break
#
#             u0 = Kinv.dot(pressure_es(u0, dc_bias, h_eff, e_flag)).squeeze()
#             niter += 1
#
#         if collapsed:
#             collapse_v = dc_bias
...
#
#
# def get_channels_from_membranes(membranes):
#     '''
#     '''
#     channels = {}
#     #channels = []
#
#     #for mem_no, mem in membranes.iteritems():
#     for mem_no, mem in enumerate(membranes):
#
#         # pull membrane info
#         center = mem['center']
#         channel_no = mem['channel_no']
#
#         if channel_no not in channels:
#         #if channel_no > len(channels):
#
#             channels[channel_no] = {}
#             channels[channel_no]['membrane_no'] = []
#             channels[channel_no]['center'] = []
#
#         channels[channel_no]['membrane_no'].append(mem_no)
#         channels[channel_no]['center'].append(center)
#
#     for ch in channels.values():
#
#         ch['center'] = np.mean(np.array(ch['center']), axis=0)
#         ch['delay'] = 0.0
#
#     test = []
