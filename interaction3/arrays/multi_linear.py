...
def init(**kwargs):
    ...
    radius_of_curvature = kwargs['radius_of_curvature']
    angle_pitch = kwargs['angle_pitch']
    ...
    angles = np.array([-angle_pitch, 0, angle_pitch])
    r = radius_of_curvature
    rr, th = np.meshgrid(r, angles)
    xx = rr * np.sin(th)
    yy = np.zeros(len(xx))
    zz =  -rr * np.cos(th) + r
    ...
    for a_pos, angle in zip(array_pos, angles):
        ...
        for e_pos in elem_pos:
            ...
            for m_pos in mem_pos:
                ...
                m['id'] = mem_counter
                m['position'] = (e_pos + m_pos + a_pos).tolist()
                membranes.append(m)
                mem_counter += 1

            # construct element
            elem = Element(id=elem_counter,
                           position=(e_pos + a_pos).tolist(),
                           membranes=membranes)
        ...
        # construct array
        array = Array(id=array_counter,
                      channels=channels,
                      position=a_pos.tolist())
        rotate_array(array, '-y', angle, a_pos)
        array_counter += 1
